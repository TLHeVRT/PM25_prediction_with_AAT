...
def retrieve_random_global_sequences(
        starts_batch,
        bank_station_ids,
        bank_starts,
        x_data,
    ...
    """Randomly select temporally valid auxiliaries from the global bank."""
    total_length = T_long + T_pred
    device = x_data.device
    selected_positions = []

    for query_start in starts_batch:
        ...
        positions = torch.tensor(
            offsets,
            dtype=torch.long,
            device=bank_starts.device,
        ) + left
        selected_positions.append(positions)

    selected_positions = torch.stack(selected_positions, dim=0)
    auxiliary_starts = bank_starts[selected_positions].to(device)
    auxiliary_station_ids = bank_station_ids[selected_positions].to(device)
...
def prepare_global_index_bank(
    ...
    """Build one start-sorted four-year index bank across all stations."""
    ...
    for split_name, offset in split_specs:
        ...
        if station_count is None:
            station_count = len(valid_starts)

        for station_id, starts in enumerate(valid_starts):
            if starts.numel() == 0:
                continue
            start_chunks.append(starts.to(dtype=torch.long) + offset)
            station_id_chunks.append(
                torch.full_like(starts, station_id, dtype=torch.long)
            )

    all_starts = torch.cat(start_chunks, dim=0)
    all_station_ids = torch.cat(station_id_chunks, dim=0)
    sort_order = torch.argsort(all_starts)
    all_starts = all_starts[sort_order]
    all_station_ids = all_station_ids[sort_order]
    print(
        "Prepared global random index bank: "
        f"stations={station_count}, records={all_starts.numel()}."
    )
    return all_station_ids, all_starts
...
def train_main_model(
    ...
    bank_stids_global, bank_starts_global = index_bank
        ...
        for batch_idx, (short_segs, short_masks, future_mets, targets, station_ids, starts_batch) in enumerate(
            ...
            with torch.no_grad():
                random_matched_seqs, auxiliary_station_ids = (
                    retrieve_random_global_sequences(
                        starts_batch,
                        bank_stids_global,
                        bank_starts_global,
                        global_x_data,
...
def evaluate_random_test_subset(
    ...
    bank_stids, bank_starts = index_bank
    ...
    with torch.inference_mode():
        for short_segs, short_masks, future_mets, targets, station_ids, starts_batch in test_loader:
            ...
            matched_seqs, matched_station_ids = retrieve_random_global_sequences(
                starts_batch_global,
                bank_stids,
                bank_starts,
                global_x_data,
...
def main():
    raise SystemExit(
        "Use `python run_parallel_pipeline_global_random.py "
        "--seeds SEED [SEED ...]`."
    )
