...
OUTPUT_ROOT = "parallel_runs_global_random"
TEST_FRACTION = 0.1
...
def run_complete_pipeline(run_id, seed, output_root, cpu_threads):
    """Run the global random-index ablation for one seed."""
    run_dir = _run_directory(output_root, run_id, seed)
    ...
    from run_models_global_random import (
    ...
    print(
        ...
        f"CPU threads: {cpu_threads}; strategy: global random."
    )
    ...
    print(
        ...
        "Preparing global random index bank..."
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run seeded global random-index ablations."
    )
    parser.add_argument(
        "--seeds",
        nargs="+",
...
def main():
    ...
    print(
        f"Scheduling {len(runs)} global random run(s): "
    ...
    for wave_start in range(0, len(runs), args.max_parallel):
        ...
        for run_id, seed in wave:
            ...
            process = mp.Process(
                ...
                name=f"global_random_{run_id}_seed{seed}",
            )
            process.start()
    ...
    if failed:
        raise SystemExit(f"Global random pipelines failed: {failed}")
    print(f"All {len(runs)} global random pipelines completed successfully.")
