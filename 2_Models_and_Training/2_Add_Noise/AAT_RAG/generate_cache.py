...
from mult_model import DataSet, get_base_skill_site_indices
...
def get_split_cache_paths(output_dir, T_short=144, pred_len=48, seed=None):
    ...
    return {
        ...
        "aux_test": os.path.join(
            cache_dir,
            f"dataset_cache_aux_test_y4_T{T_short}_P{pred_len}{seed_suffix}.pt",
        ),
    }
...
def generate_split_caches(
    ...
    for skill_group in ("low", "mid", "high"):
        print(f"Processing {skill_group} test set...")
        generate_and_save_cache(
            test_met, test_pol, test_mask, T_short, pred_len,
            cache_paths["test"][skill_group],
            pol_mean=data_set.pol_mean, pol_std=data_set.pol_std,
            station_indices=test_station_indices[skill_group]
        )

    print("Processing auxiliary-library test-year data...")
    generate_and_save_cache(
        test_met, test_pol, test_mask, T_short, pred_len, cache_paths["aux_test"],
        pol_mean=data_set.pol_mean, pol_std=data_set.pol_std,
        station_indices=train_station_indices
    )
