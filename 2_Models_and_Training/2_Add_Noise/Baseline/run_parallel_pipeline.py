...
class RandomTestSubset:
    ...
    def __getitem__(self, index):
        ...
        return short_seg, short_mask, future_mets, targets
...
def evaluate_test_subset(worker_dir, data_path, best_model_path, seed):
    ...
    from run_models import build_model, pm25_no_trend_loss_components
    ...
    test_cache_files = {
        group: os.path.join(
            worker_dir,
            f"dataset_cache_test_{group}_1y_"
            f"T{T_short}_P{pred_len}_seed_{seed}.pt"
        )
    ...
    for skill_group in ("low", "mid", "high"):
        ...
        with torch.no_grad():
            for short_segs, short_masks, future_mets, targets in test_loader:
                ...
                preds = predictor(
                    short_segs, future_met=future_mets, mask=short_masks
                )
    ...
    return {
        row['skill_group']: {**row, 'report_file': report_path}
        for row in rows
    }
...
def run_worker(worker_id, seed, cpu_threads):
    ...
    print(
        ...
        f"({TEST_SAMPLE_FRACTION:.0%} test sample): {worker_dir}"
    )
