...
def build_memory_bank(
    ...
    print(f"Building memory bank (Total Stations: {databank.N_stations})...")
    encoder.eval()
    encoder.to(device)
...
def retrieve_top10_sequences(curr_vecs, starts_batch, bank_vectors, bank_norms, bank_stids, bank_starts, x_data, T_long, T_pred):
    """Return the ten nearest temporally valid auxiliary sequences per query."""
    B = curr_vecs.shape[0]
    ...
    # The full bank is FP16. Only the sampled candidate subset is promoted to
    # FP32 so L2 ranking keeps FP32 arithmetic without a full-bank FP32 copy.
    sub_bank_vecs = bank_vectors[rand_idx].float()
    ...
    for i in range(0, B, chunk_size):
        ...
        _, topk_idx_chunk = torch.topk(
            dists_chunk, k=k_num, dim=1, largest=False
        )
...
class PredictionDatasetWithFuture(Dataset):
    ...
    def __init__(self, met_data, pol_data, mask_data, cache_file,
        ...
        # Optionally restrict which cached windows may serve as main sequences.
        # The underlying cache remains unchanged and can still be used to build
        # the complete retrieval bank.
        if minimum_start is not None:
            self.valid_starts_per_station = [
                starts[starts >= minimum_start]
                for starts in self.valid_starts_per_station
            ]
            self.eligible_stations = torch.tensor(
                [
                    station_id
                    for station_id, starts in enumerate(self.valid_starts_per_station)
                    if len(starts) > 0
                ],
                dtype=torch.long,
            )
...
def prepare_global_memory_bank(
    ...
    """Build one pipeline's private four-year retrieval bank."""
    ...
    bank_cache_train = os.path.join(
        cache_dir, f"memory_bank_train_y1_y2_T{T_short}{seed_suffix}.pt"
    )
    bank_cache_val = os.path.join(
        cache_dir, f"memory_bank_val_y3_T{T_short}{seed_suffix}.pt"
    )
    bank_cache_test = os.path.join(
        cache_dir, f"memory_bank_test_y4_T{T_short}{seed_suffix}.pt"
    )

    print("Preparing Train Bank...")
    train_vectors, train_station_ids, train_starts = build_memory_bank(
        train_pred_ds, light_encoder, device, bank_cache_train,
        batch_size=8192
    )
    print("Preparing Val Bank...")
    val_vectors, val_station_ids, val_starts = build_memory_bank(
        val_pred_ds, light_encoder, device, bank_cache_val,
        batch_size=8192
    )
    print("Preparing Test Bank...")
    test_vectors, test_station_ids, test_starts = build_memory_bank(
        test_pred_ds, light_encoder, device, bank_cache_test,
        batch_size=8192
    )

    # Convert validation/test start indices to the common four-year timeline.
    val_starts = val_starts + train_end
    test_starts = test_starts + val_end

    all_vectors = torch.cat([train_vectors, val_vectors, test_vectors], dim=0)
    all_station_ids = torch.cat(
        [train_station_ids, val_station_ids, test_station_ids], dim=0
    )
    all_starts = torch.cat([train_starts, val_starts, test_starts], dim=0)

    del train_pred_ds, val_pred_ds, test_pred_ds
    del train_met, val_met, test_met
    light_encoder.cpu()
    torch.cuda.empty_cache()
    return all_vectors, all_station_ids, all_starts
...
def train_main_model(
    ...
    bank_norms_global = compute_bank_norms(bank_vecs_global)
    ...
    train_pred_ds = PredictionDatasetWithFuture(
        ...
        # Firstyear_out ablation: only year 2 can be sampled as a main
        # training sequence. Year 1 remains available in the retrieval bank.
        minimum_start=HOURS_PER_YEAR,
    )
...
def evaluate_random_test_subset(
    ...
    bank_vectors, bank_stids, bank_starts = bank_global
...
def main():
    raise SystemExit(
        "Use `python run_parallel_pipeline.py --seeds SEED [SEED ...]`."
    )
