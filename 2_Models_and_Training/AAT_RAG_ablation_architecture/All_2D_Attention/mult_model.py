...
class ShortTermPredictorWithFuture(nn.Module):
    """All-2D-attention ablation of the Evoformer-based heavy predictor."""
    ...
    def __init__(
        ...
        # First half: full 2D attention, unchanged from the original model.
        ...
        # The auxiliary reconstruction head remains between the two attention halves.
        self.aux_reconstruct_head = nn.Linear(d_model, 1)

        # Ablation: replace the original second-half one-way target-to-aux
        # attention with the same full 2D attention used in the first half.
    ...
    def forward(self, x_short, profile, future_met, aux_profiles=None, mask=None, matched_hist=None):
        ...
        # First-half full 2D attention updates
        ...
        # Middle auxiliary reconstruction, unchanged from the original model
        aux_reconstruct = self.aux_reconstruct_head(aux_feat).squeeze(-1)  # [B, 10, 192]

        # Second-half full 2D attention updates (the only architectural ablation)
        grid = torch.cat([target_feat, aux_feat], dim=1)
